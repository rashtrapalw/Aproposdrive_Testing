...
const PRODUCTS: Product[] = [
  {
    ...
    specs: [
      ...
      { icon: Activity,   label: "Peak Torque", value: "200 Nm",  color: "#00813A", angle: 330 },
      { icon: TrendingUp, label: "Efficiency",  value: "95%",     color: "#006B6B", angle: 30  },
      { icon: Weight,     label: "Weight",      value: "7.5 kg",  color: "#00C853", angle: 90  },
      { icon: Shield,     label: "Protection",  value: "IP67",    color: "#006B6B", angle: 150 },
      { icon: Battery,    label: "Voltage",     value: "48/60V",  color: "#00813A", angle: 210 },
    ],
    variants: [
  ...
  },
  {
    ...
    specs: [
      { icon: Zap,         label: "Peak Current", value: "280A",     color: "#006B6B", angle: 270 },
      { icon: Activity,    label: "Power Range",  value: "3–7 kW",   color: "#00897B", angle: 330 },
      { icon: Thermometer, label: "Temp Range",   value: "±90°C",    color: "#00813A", angle: 30  },
      { icon: Shield,      label: "Protection",   value: "IP67",     color: "#006B6B", angle: 90  },
      { icon: Wifi,        label: "Comms",        value: "CAN+USB",  color: "#00813A", angle: 150 },
      { icon: Cpu,         label: "Control",      value: "PMSM FOC", color: "#00897B", angle: 210 },
    ],
    variants: [
  ...
  return (
    <div style={{ position: "relative", width: C, height: C, flexShrink: 0 }}>
      {[R * 2 + 20, R * 2 + 90].map((d, i) => (
        <motion.div key={d}
          style={{
            ...
            borderRadius: "50%",
            border: `1px solid ${product.accent}${i === 0 ? "35" : "18"}`,
            pointerEvents: "none",
          }}
          ...
          return (
            <g key={s.label}>
              <line
                ...
                stroke={s.color} strokeWidth="1" strokeDasharray="4 5" strokeOpacity="0.5"
              />
              <circle
                cx={CX + Math.cos(rad) * (R + CW / 2 + 6)}
                cy={CY + Math.sin(rad) * (R + CH / 2 + 6)}
                r="3" fill={s.color} opacity="0.8"
              />
            </g>
          );
        })}
      ...
      <div style={{
        ...
        left: CX - (IMG + 200) / 2, top: CY - (IMG + 200) / 2,
        borderRadius: "50%",
        background: `radial-gradient(circle, ${product.accent}18 0%, ${product.accent}08 40%, transparent 70%)`,
        filter: "blur(48px)",
        pointerEvents: "none",
      }} />
      ...
      <motion.div
        style={{
          position: "absolute",
          width: IMG - 40, height: IMG - 40,
          left: CX - (IMG - 40) / 2, top: CY - (IMG - 40) / 2,
          borderRadius: "50%",
          background: `radial-gradient(circle, rgba(255,255,255,0.75) 0%, ${product.accent}30 45%, transparent 70%)`,
          filter: "blur(18px)",
          pointerEvents: "none",
        }}
      ...
      <div style={{
        ...
        top: CY + IMG / 2 - 30,
        borderRadius: "50%",
        background: `radial-gradient(ellipse, ${product.accent}40 0%, transparent 70%)`,
        filter: "blur(14px)",
        pointerEvents: "none",
      }} />
      ...
      <motion.div style={{
        ...
        <ImageWithFallback
          ...
          style={{
            ...
            filter: `drop-shadow(0 0 24px ${product.accent}70) drop-shadow(0 12px 40px ${product.accent}45) drop-shadow(0 2px 8px rgba(0,0,0,0.12))`,
          }}
        />
      </motion.div>
    ...
    >
      {hov && (
        <div style={{
          ...
          background: `radial-gradient(circle at 50% 0%, ${spec.color}14, transparent 70%)`,
        }} />
      )}
      <div style={{
        width: 30, height: 30, borderRadius: 8, flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
        background: `${spec.color}14`, border: `1px solid ${spec.color}30`,
      }}>
...
function FeaturePill({ icon: Icon, text, color, accentText, delay, mobile = false }: { icon: LucideIcon; text: string; color: string; accentText: string; delay: number; mobile?: boolean }) {
  ...
  return (
    ...
    >
      <div style={{
        width: mobile ? 30 : 28, height: mobile ? 30 : 28,
        borderRadius: 8, flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
        background: `${color}18`, border: `1px solid ${color}35`,
      }}>
        <Icon style={{ width: mobile ? 14 : 13, height: mobile ? 14 : 13, color }} />
      </div>
      <span style={{
...
function ProductBlock({ product, index }: { product: Product; index: number }) {
  ...
  const LabelRow = (
    ...
    >
      ...
      <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.28em", color: product.accentText }}>
        Product {product.num}
      </span>
    </motion.div>
  );
  ...
  const FeaturesList = (
    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 6 : 3 }}>
      {product.features.map((f, i) => (
        <FeaturePill key={i} icon={f.icon} text={f.text} color={product.accent} accentText={product.accentText} delay={i * 0.04} mobile={isMobile} />
      ))}
    </div>
  );
  ...
  const RightPanel = (
    <>
      {product.variants.map((v, i) => <VariantCard key={v.name} variant={v} accent={product.accent} accentText={product.accentText} delay={i * 0.08} />)}
      <SpecsAccordion headers={product.tableHeaders} rows={product.tableRows} accent={product.accent} accentText={product.accentText} delay={0.16} />
      <a href="/products" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 12, fontWeight: 700, color: product.accentText, fontFamily: "'DM Sans',sans-serif", textDecoration: "none" }}>
        ...
        <div style={{ width: 22, height: 22, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: `${product.accent}14`, border: `1px solid ${product.accent}30` }}>
          <ArrowRight style={{ width: 11, height: 11, color: product.accentText }} />
        </div>
      </a>
    </>
  );
  ...
  if (isMobile) {
    return (
      <div ref={ref} style={{ marginBottom: 60 }}>
        {LabelRow}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }} transition={{ duration: 0.5 }} style={{ marginBottom: 20 }}>
          {TitleBlock(22)}
        </motion.div>
        <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={inView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.9 }} transition={{ duration: 0.6, delay: 0.08 }} style={{ width: "100%", marginBottom: 20 }}>
          <ScaleBox canvasSize={C}><OrbitalShowcase product={product} idx={index} /></ScaleBox>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 12 }} transition={{ duration: 0.5, delay: 0.12 }} style={{ marginBottom: 16 }}>
          {FeaturesList}
        </motion.div>
        {PlatformsBlock && <div style={{ marginBottom: 16 }}>{PlatformsBlock}</div>}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 12 }} transition={{ duration: 0.5, delay: 0.16 }} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {RightPanel}
        </motion.div>
      </div>
    );
  ...
  const DetailsSection = (
    <motion.div
      ...
      animate={inView ? { opacity: 1, x: 0 } : { opacity: 0, x: reverseLayout ? 24 : -24 }}
      transition={{ duration: 0.5, delay: 0.05 }}
      style={{ width: isTablet ? 420 : 500, flexShrink: 0, display: "flex", flexDirection: "column", gap: 14 }}
    >
      {TitleBlock(isTablet ? 20 : 24)}
      {FeaturesList}
      {PlatformsBlock}
      {product.variants.map((v, i) => <VariantCard key={v.name} variant={v} accent={product.accent} accentText={product.accentText} delay={i * 0.08} />)}
      <SpecsAccordion headers={product.tableHeaders} rows={product.tableRows} accent={product.accent} accentText={product.accentText} delay={0.16} />
      <a href="/products" style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 12, fontWeight: 700, color: product.accentText, fontFamily: "'DM Sans',sans-serif", textDecoration: "none", marginTop: 4 }}>
        Explore Full Details
        <div style={{ width: 22, height: 22, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: `${product.accent}14`, border: `1px solid ${product.accent}30` }}>
          <ArrowRight style={{ width: 11, height: 11, color: product.accentText }} />
        </div>
  ...
  return (
    <div ref={ref} style={{ marginBottom: 90 }}>
      {LabelRow}
      <div style={{ display: "flex", flexDirection: reverseLayout ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between", gap: isTablet ? 28 : 48, minWidth: 0 }}>
        {DetailsSection}
        {ImageSection}
      </div>
    </div>
